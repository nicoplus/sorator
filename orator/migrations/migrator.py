...
from .schema_gen import parse_sql_syntax
...
class Migrator:
    ...
    def run(self, path, pretend=False):
        ...
        with open(dump_path, 'w') as fd:
            output_buffer = parse_sql_syntax(output_buffer)
            fd.write('\n\n'.join(output_buffer))
    ...
    def _run_up(self, path, migration_file, batch, pretend=False):
        ...
        self._note(decode('[<info>OK</>] <info>Migrated</info> ') +
    ...
    def _run_down(self, path, migration, pretend=False):
        ...
        self._note(decode(
            '[<info>OK</>] <info>Rolled back</info> ') + '<fg=cyan>%s</>' %
                   migration_file)
