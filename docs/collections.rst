...
The ``serialize`` method converts the collection into a ``list``. If the collection’s
values are ``ORM`` models, the models will also be converted to dictionaries:
...
.. warning::
  serialize also converts all of its nested objects. If you want to get the
  underlying items as is, use the all method instead.
