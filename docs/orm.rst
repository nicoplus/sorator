...
.. note::
    ...
    If you want to apply permanent query conditions on your relationships you can
    do so by returning a Builder instance instead of a Model subclass.

    For example, let’s say you want all comments of a user to be ordered by date
    of creation in descending order and only the id and title columns of each comment:
...
.. note::
  ...
  Orator no longer officially supports Arrow instances as datetimes.

  It will now only supports Pendulum instances which are a drop-in replacement
  for the standard datetime class.


By default, the ORM will convert the ``created_at`` and ``updated_at`` columns to instances of `Pendulum <https://pendulum.eustace.io/>`_,
